...
public class EmailNotifier {
	...
	File gmailCredentialFile = new File("outgoingGmailCreds.txt");	
	...
	private void parseGmailCredentialsFromFile(){
		Scanner credentialScanner;
		try {
			credentialScanner = new Scanner(gmailCredentialFile);
			while(credentialScanner.hasNextLine()){
				String currentLine = credentialScanner.nextLine();
				if(!currentLine.startsWith("//")){
					gmailUsername = currentLine;
					gmailPassword = credentialScanner.nextLine();
					break;
				}
			}
	...
	private void buildEmailMetadata(){
		try {
			...
			if(this.attachmentFilePath != null) buildAttachmentBodyPart();
			buildMessageData();	
	...
	private void createSession(){
		javaMailSession = Session.getInstance(mailSessionProperties,
	...
	private void buildMessageData() throws AddressException, MessagingException{
		...
		javaMailMessage.setRecipients(Message.RecipientType.TO,
	...
	public void setAttachmentFilePath(String attachmentFilePath){
		if(attachmentFilePath != null && !new File(attachmentFilePath).exists()) throw new IllegalArgumentException("File " + attachmentFilePath + " can't be found");
